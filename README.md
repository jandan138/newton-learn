...
3. 开始填内容前先对照 `conventions/`、`templates/` 与 `references/`，保证符号、结构、资料记录和章节入口一致。
4. Newton 源码引用统一按 `conventions/source-refs.md` 里的 source-ref 格式书写，避免模糊指代。

## 仓库布局

下表描述的是本仓库当前的 bootstrap 结构；这些目录和入口文件已经创建，可直接按表中的职责继续推进。

| 路径 | 作用 |
|------|------|
| `conventions/` | 已创建的跨章节统一约定目录，包含符号、术语、source-ref、GPU 和 diffsim 记录规范。 |
| `templates/` | 已创建的章节模板目录，用来统一 README / principle / source-walkthrough 等文件结构。 |
| `references/` | 已创建的外部资料目录，用来整理资料导航、BibTeX 条目占位、论文摘要与链接。 |
| `chapters/` | 已创建的 17 章主学习骨架目录，按物理层次和 Newton 心智模型推进。 |
