...
```python
self.joint_parent.append(parent)  # 记住这条 joint 的 parent body
self.joint_child.append(child)  # 记住这条 joint 的 child body
self.joint_X_p.append(parent_xform)  # parent 侧关节局部 frame
self.joint_X_c.append(child_xform)  # child 侧关节局部 frame
...
self.joint_q_start.append(self.joint_coord_count)  # 这条 joint 的 position slice 起点
self.joint_qd_start.append(self.joint_dof_count)  # 这条 joint 的 velocity / dof slice 起点
```

真正声明“这一串 joint 组成一条 articulation”的地方也很直接：

```python
self.articulation_start.append(sorted_joints[0])  # 记住这条 articulation 从哪个 joint 开始
self.articulation_label.append(label or f"articulation_{articulation_idx}")  # 给这条树一个可读标签
...
for joint_idx in joints:
    self.joint_articulation[joint_idx] = articulation_idx  # 回填 joint -> articulation 的归属
```

`finalize()` 再把这些起点补上 sentinel：

```python
joint_q_start = copy.copy(self.joint_q_start)  # 先复制已有的 position slice 起点
joint_q_start.append(self.joint_coord_count)  # 末尾 sentinel = 总坐标数
joint_qd_start = copy.copy(self.joint_qd_start)  # 先复制已有的 velocity slice 起点
joint_qd_start.append(self.joint_dof_count)  # 末尾 sentinel = 总自由度数
articulation_start = copy.copy(self.articulation_start)  # 先复制每条 articulation 的起点
articulation_start.append(self.joint_count)  # 末尾 sentinel = 总 joint 数
```
...
如果你现在还会把 articulation 想成“Python 对象树直接喂给 solver”，先不要继续。第一遍最重要的是先接受：工程实现里它先被压成 flat slice layout。
...
这里先保留声明块干净，因为这一段的重点是看 joint/body 两层槽位并列出现，逐行注释反而会把对照关系打散。
...
```python
s.body_q = wp.clone(self.body_q, requires_grad=requires_grad)  # 复制 body pose 快照
s.body_qd = wp.clone(self.body_qd, requires_grad=requires_grad)  # 复制 body velocity 快照
...
s.joint_q = wp.clone(self.joint_q, requires_grad=requires_grad)  # 复制 generalized position 快照
s.joint_qd = wp.clone(self.joint_qd, requires_grad=requires_grad)  # 复制 generalized velocity 快照
```
...
如果你现在还会把 `joint_q / joint_qd` 和 `body_q / body_qd` 当成同一层状态的两套名字，先停一下。chapter 05 的主线之一，就是先把这两层状态拆开。
...
```python
q_start = joint_q_start[i]  # 先切到这个 joint 的 position 段
qd_start = joint_qd_start[i]  # 再切到它的 velocity / dof 段
...
if type == JointType.REVOLUTE:
    axis = joint_axis[qd_start]  # 这一自由度对应的转轴

    q = joint_q[q_start]  # 当前关节角
    qd = joint_qd[qd_start]  # 当前关节角速度

    X_j = wp.transform(wp.vec3(), wp.quat_from_axis_angle(axis, q))  # joint 位姿增量
    v_j = wp.spatial_vector(wp.vec3(), axis * qd)  # joint 速度增量
```

然后再把 joint motion 沿拓扑链组合到 child body：

```python
X_wpj = X_pj  # 先从 parent 侧 joint frame 出发
if parent >= 0:
    X_wp = body_q[parent]  # 取 parent 当前 world pose
    X_wpj = X_wp * X_wpj  # 把 joint frame 接到 world 下

X_wcj = X_wpj * X_j  # 应用 joint 自身的位姿增量
X_wc = X_wcj * wp.transform_inverse(X_cj)  # 换回 child body frame
...
body_q[child] = X_wc  # 写回 child 的 world pose
body_qd[child] = origin_twist_to_com_twist(v_wc_origin, X_wc, body_com[child])  # 再改写成以 COM 为参考点的 body velocity
```
...
如果你现在还会把 FK 读成“只是把角度变成姿态”，先不要继续。这里真正发生的是 joint-space 状态一路展开成 body-space 状态。
...
先给这一段加一个最小符号表，不然第一次看到 solver-side spatial buffers 很容易失去方向：

- `body_I_m`: body 在质心系下的 spatial inertia
- `body_X_com`: body origin 到 COM 的 transform
- `joint_S_s`: joint motion subspace，也就是“这个 joint 允许运动的方向基”
- `body_v_s`: solver 内部的 body spatial velocity buffer
- `body_f_s`: solver 内部的 body spatial force buffer
- `body_I_s`: solver 内部继续递推用的 spatial inertia buffer

这里先保留两段 spatial 代码的原貌，因为第一遍更需要看清 buffer 形状和数据流，而不是逐行翻译空间代数。
...
如果你现在还会把 `body_I_m / joint_S_s / body_v_s` 这些名字看成互不相干的新黑箱，先停一下。第一遍只要先记住：它们都是 solver 为 articulation 递推准备的内部 spatial buffers。
...
```python
wp.launch(
    eval_rigid_fk,  # 先按 articulation 批量刷新 FK
    dim=model.articulation_count,  # 每条 articulation 一次
    inputs=[
        model.articulation_start,  # articulation 的 joint 范围
        model.joint_type,
        model.joint_parent,
        model.joint_child,
        model.joint_q_start,
        model.joint_qd_start,
        state_in.joint_q,  # 当前 generalized positions
        ...
    ],
    outputs=[state_in.body_q, state_aug.body_q_com],  # 写回 body pose 和 solver 侧 COM pose
)
```
...
```python
wp.launch(
    kernel=integrate_generalized_joints,  # 先在 joint-space 完成积分
    dim=model.joint_count,  # 每个 joint 更新一次
    inputs=[
        model.joint_type,
        model.joint_q_start,
        model.joint_qd_start,
        ...
        state_in.joint_q,  # 当前 generalized positions
        state_aug.joint_qd_internal_in,  # 当前内部 joint velocities
        state_aug.joint_qdd,  # solve 出来的 joint accelerations
        dt,  # 时间步长
    ],
    outputs=[state_out.joint_q, state_aug.joint_qd_internal_out],  # 产出下一拍 generalized state
)
...
eval_fk_with_velocity_conversion(
    ...
    state_out,
)  # 再把 joint-space 结果展开回公共 body state
```
...
如果你现在还会把 `SolverFeatherstone.step()` 想成“完全重造了一套外部状态”，先不要继续。更准确的读法是：它只是在同一条 articulation handoff 上，多建了一层 solver 内部工作区。
